...
SimpleObject3D *FigureBuilder::initStair(const QImage &texture, QVector3D center1, QVector3D center2, double r1, double r2, double step)
{
    double zNormal;
    double delta = r2 - r1;
    if(delta > 0)
    ...
    for(float i = 0; i <= 2 * M_PI + step; i += step)
    {
        disk1Points.push_back(QVector3D(disk1Center.position.x(), disk1Center.position.y() + sin(i) * r1, disk1Center.position.z() + cos(i) * r1));
        disk1Normals.push_back(QVector3D(0.0, sin(i), cos(i)));
    }
    ...
    for(float i = 0; i <= 2 * M_PI + step; i += step)
    {
        disk2Points.push_back(QVector3D(disk2Center.position.x(), disk2Center.position.y() + sin(i) * r2, disk2Center.position.z() + cos(i) * r2));
    }
